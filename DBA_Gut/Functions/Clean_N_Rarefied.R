...
# --------------------------------------------------
# Output(s) :
# OTU_count_Histogram.png : Raster graphical (repartition of reads per sample)
# over_min_reads.csv : Table (sample over the threshold)
# under_min_reads.csv : Table (sample below the threshold )
# Root.csv : Table (OTU identified as Root)
# Chloroplast.csv : Table (OTU identified as chloroplast)
# Rarefaction_curve.png : Raster graphics (Rarefaction curve)
# missing_meta_sample.txt : text (removed and missing samples)
...
Clean_N_Rarefied = function(min_reads, rare_curve){
  ...
  # Download Meta data
  metadata <- read.csv2("./Datasets/Metadata_Gut_DBA.csv", header=T, sep = ";", na.strings = "")
  ...
  # Select the informative variables
  metadata <- metadata %>% select(ID, parasite_strain, mouse_strain, 
  ...
  # Download Datasets 
  OTU_Table <- read.delim("./Datasets/OTU_Table.txt", header = TRUE, sep = "\t", dec = ".")
  Taxonomy <- read.delim("./Datasets/Taxonomy.txt", header = TRUE, sep = "\t", dec = ".")
  
  # Rename X.OTU.ID colomn by OTU
  OTU_Table <- OTU_Table %>% dplyr::rename(OTU = X.OTU.ID)
  
  # Remove "_paired_M05485" from colnames
  colnames(OTU_Table) <- OTU_Table %>% colnames() %>% gsub("_paired_M04889", "", .)
  ...
  png("./Checkpoints_Cleaning/OTU_count_Histogram.png", units="in", width=7, height=5, res=1200)
  OTU_Table %>%  colSums() %>% 
    hist(breaks=50, main="Lung Seq 2", 
  ...
  OTU_Table <- OTU_Table %>% select(all_of(row.names(over_min_reads)))
  
  # Save csv file 
  write.csv2(over_min_reads, "./Checkpoints_Cleaning/over_min_reads.csv")
  write.csv2(under_min_reads, "./Checkpoints_Cleaning/under_min_reads.csv")
  ...
  # Save the infos w
  write.csv2(Root, "./Checkpoints_Cleaning/Root.csv")
  write.csv2(Chloro, "./Checkpoints_Cleaning/Chloroplast.csv")
  ...
  for (taxo_rank in c("domain", "phylum", "class",
    ...
    OneTax_Table <- aggregate(OneTax_Table %>% select(-Taxo),
    ...
    # Checks
    nrow(anti_join(metadata, OneTax_Table, by="ID")) 
    no_seq <- anti_join(metadata, OneTax_Table, by="ID") %>% select(ID) # 2 samples no seq
    no_meta <- anti_join(OneTax_Table, metadata, by="ID") %>% row.names() # 3 samples no metadata
    dim(exp_OTU)
    
    # Note the missing infos
    cat("Sample(s) with NO sequences : ", no_seq$ID,
        "Sample(s) with NO metadata : ", no_meta, sep="\n",
        file = "./Checkpoints_Cleaning/missing_meta_sample.txt")
    
    # Remove mouse cage information in OTU table
    exp_OTU <- exp_OTU %>% dplyr::select(-mouse_cage)
